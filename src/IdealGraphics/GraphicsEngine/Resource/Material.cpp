...
void Ideal::Material::Create(std::shared_ptr<D3D12Renderer> Renderer)
{
...
void Ideal::Material::BindToShader(std::shared_ptr<D3D12Renderer> Renderer)
{
	if (m_diffuseTexture)
	{
		...
		Renderer->GetCommandList()->SetGraphicsRootDescriptorTable(STATIC_MESH_ROOT_DESCRIPTOR_TABLE_PIXEL_SRV_INDEX_DIFFUSE, diffuseGPUAddress);
	}
}
