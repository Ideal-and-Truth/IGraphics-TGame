...
void Ideal::IdealRenderScene::CreateStaticMeshPSO2(std::shared_ptr<IdealRenderer> Renderer)
{
	...
	CD3DX12_DESCRIPTOR_RANGE1 rangePerFacegroup[2];
	rangePerFacegroup[0].Init(D3D12_DESCRIPTOR_RANGE_TYPE_CBV, 1, 1);	// b1 : Material	// mesh마다 다른 material을 가지고 있으니 갈아 끼워야한다.
	rangePerFacegroup[1].Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 3, 0);	// t0 ~ t2 : Shader Resource View

	//-------------------Parameter--------------------//
	CD3DX12_ROOT_PARAMETER1 rootParameters[2];
	rootParameters[0].InitAsDescriptorTable(_countof(rangePerObj), rangePerObj, D3D12_SHADER_VISIBILITY_ALL);
	rootParameters[1].InitAsDescriptorTable(_countof(rangePerFacegroup), rangePerFacegroup, D3D12_SHADER_VISIBILITY_ALL);
