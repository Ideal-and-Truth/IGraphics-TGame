...
void Ideal::IdealMaterial::SetEmissiveMap(std::shared_ptr<Ideal::ITexture> Texture)
{
	Ideal::Singleton::RayTracingFlagManger::GetInstance().SetTextureChanged();
...
void Ideal::IdealMaterial::CopyHandleToRayTracingDescriptorTable(ComPtr<ID3D12Device> Device)
{
	...
	if (!m_emissiveTexture.expired())
		Device->CopyDescriptorsSimple(1, m_emissiveTextureInRayTracing.GetCpuHandle(), m_emissiveTexture.lock()->GetSRV().GetCpuHandle(), D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
}
