...
void Ideal::BloomPass::InitBloomPass(ComPtr<ID3D12Device> Device, std::shared_ptr<Ideal::ResourceManager> ResourceManager, std::shared_ptr<Ideal::D3D12Shader> DownSampleShader, std::shared_ptr<Ideal::D3D12Shader> BlurShader, std::shared_ptr<Ideal::IdealMesh<SimpleVertex>> QuadMesh, uint32 Width, uint32 Height)
{
	...
	ResourceManager->CreateEmptyTexture2D(m_downSampleTexture0, Width * m_threshold, Height * m_threshold, DXGI_FORMAT_R16G16B16A16_FLOAT, textureFlag, L"BlurTexture0");
	// Texture 만들기 -> 블러처리
	ResourceManager->CreateEmptyTexture2D(m_blurTexture[0], Width * m_threshold, Height * m_threshold, DXGI_FORMAT_R16G16B16A16_FLOAT, textureFlag, L"BlurTexture0");
	ResourceManager->CreateEmptyTexture2D(m_blurTexture[1], Width * m_threshold, Height * m_threshold, DXGI_FORMAT_R16G16B16A16_FLOAT, textureFlag, L"BlurTexture0");
...
void Ideal::BloomPass::Resize(uint32 Width, uint32 Height, std::shared_ptr<DeferredDeleteManager> DeferredDeleteManager, std::shared_ptr<Ideal::ResourceManager> ResourceManager)
{
	...
	for (uint32 i = 0; i < 2; ++i)
	{
		...
		ResourceManager->CreateEmptyTexture2D(m_blurTexture[i], Width * m_threshold, Height * m_threshold, DXGI_FORMAT_R16G16B16A16_FLOAT, textureFlag, L"BlurTexture0");
	}
}
