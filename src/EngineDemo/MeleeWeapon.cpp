...
void MeleeWeapon::OnTriggerEnter(Truth::Collider* _other)
{
	if (m_isAttacking && m_canHit)
	{
		if (m_player && _other->GetOwner().lock() != m_owner.lock()->m_parent.lock())
		{
			...
			{
				if (_other->GetOwner().lock()->GetComponent<BossAnimator>().lock())
				{
					if (_other->GetOwner().lock()->GetComponent<BossAnimator>().lock()->GetTypeInfo().GetProperty("playOnce")->Get<bool>(_other->GetOwner().lock()->GetComponent<BossAnimator>().lock().get()).Get())
					{
